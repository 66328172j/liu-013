...
import com.fc.v2.mapper.auto.TAmRuleMapper;
import com.fc.v2.model.auto.TAmRule;
import com.fc.v2.service.ITAmRuleService;

/**
 * 检验判废判据 Service业务层处理（rule-eval 形状）
 *
 * @author fuce
 * @date 2026-09-14
 */
@Service
public class TAmRuleServiceImpl implements ITAmRuleService {

    /** 优先级降序；并列按 ruleCode 降序（确定性） */
    private static final Comparator<TAmRule> PRIORITY_THEN_CODE_DESC = new Comparator<TAmRule>() {
        @Override
        public int compare(TAmRule a, TAmRule b) {
    ...
    private TAmRuleMapper amRuleMapper;
    ...
    @Override
    public TAmRule selectTAmRuleById(Long id) {
        return this.amRuleMapper.selectById(id);
    }

    @Override
    public List<TAmRule> listAvailable(Date at) {
        if (at == null) {
            return new java.util.ArrayList<TAmRule>();
        }
        return this.amRuleMapper.selectList(new QueryWrapper<TAmRule>().eq("del_flag", 0));
    }
    ...
    public int evaluate(String ruleCode, BigDecimal input, Date at) {
        ...
        TAmRule rule = findRule(ruleCode, at);
    ...
    public int evaluateTop(BigDecimal input, Date at) {
        ...
        List<TAmRule> avail = listAvailable(at);
        if (avail.isEmpty()) {
            return 0;
        }
    ...
    public boolean usable(Long id, Date at) {
        ...
        TAmRule r = this.amRuleMapper.selectById(id);
        return r != null;
    }
    ...
    public int countAvailable(Date at) {
        ...
        return this.amRuleMapper
                .selectList(new QueryWrapper<TAmRule>().eq("del_flag", 0)).size();
    }

    private TAmRule findRule(String ruleCode, Date at) {
        List<TAmRule> hit = this.amRuleMapper.selectList(new QueryWrapper<TAmRule>()
    ...
    private BigDecimal resolveLimit(TAmRule r, int tier) {
        if (tier == 1) {
            return r.getTh1Max();
        }
        if (tier == 2) {
    ...
    /** 档案级默认阈值（明细未维护时的回退来源） */
    private BigDecimal defaultThreshold(TAmRule r, int tier) {
        return r.getTh1Max();
    }

    /** 分档：等于上限取高一档 */
    private int levelOf(TAmRule rule, BigDecimal input) {
